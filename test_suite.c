...
/* UART for test output (reuse your UART driver if available) */
extern void uart_init(void);
extern void uart_transmit_buffer(const char *str);
...
void test_report(const char *test_name, bool passed)
{
    ...
    if (passed) {
        tests_passed++;
        uart_transmit_buffer(TEST_PASS);
    } else {
        tests_failed++;
        uart_transmit_buffer(TEST_FAIL);
    }
    
    uart_transmit_buffer(test_name);
...
{
    uart_transmit_buffer("\r\n========================================\r\n");
    uart_transmit_buffer("RTOS TEST SUMMARY\r\n");
    uart_transmit_buffer("========================================\r\n");
    
    if (tests_failed == 0) {
        uart_transmit_buffer("ALL TESTS PASSED!\r\n");
    } else {
        uart_transmit_buffer("SOME TESTS FAILED!\r\n");
    }
...
{
    /* High-priority task should run more often */
...
void test_queue_sender(void)
{
    ...
    for (uint32_t i = 0; i < 10; i++) {
        msg.value = i;
        rtos_queue_send(&test_queue, &msg);
        rtos_delay(10);
    }
    
    while (1) rtos_delay(1000);
}
...
{
    bool result = rtos_queue_init(&test_queue, queue_buffer, 
                                   sizeof(test_msg_t), 5);
...
{
    rtos_task_create(test_queue_sender, 2, "QueueSend");
    rtos_task_create(test_queue_receiver, 3, "QueueRecv");
...
void test_runner(void)
{
    ...
    uart_transmit_buffer("\r\n========================================\r\n");
    uart_transmit_buffer("RTOS COMPREHENSIVE TEST SUITE\r\n");
    uart_transmit_buffer("========================================\r\n\r\n");
